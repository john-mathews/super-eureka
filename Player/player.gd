...
var dash_max_charges:= 3
var dash_current_charges:= 3
...
func _physics_process(delta: float) -> void:
	...
	#in Air	
		...
		if is_on_wall_only():
			...
			if v_direction < 0 || input_towards_wall:
				player_state_manager.set_current_state(PlayerStateManager.PlayerState.WALL_HOLD)
			elif input_away_wall:
				player_state_manager.set_current_state(PlayerStateManager.PlayerState.FALL)
				
	if (Input.is_action_just_pressed("dash") && dash_current_charges > 0) || (is_dashing && dash_time_counter <= dash_time):
		if not is_dashing:
		...
		if dash_cooldown_timer > 0 && dash_current_charges < dash_max_charges:
			dash_cooldown_timer -= delta
		elif dash_current_charges < dash_max_charges:
			dash_current_charges += 1
			dash_cooldown_timer = dash_refresh_time
			
	if Input.is_action_just_pressed("jump") && jump_counter < max_jumps:
		jump_counter += 1
		jump_held_time = 0.0
		if is_on_wall_only():
	...
	SignalHandler.update_dashes.emit(dash_current_charges)	
	SignalHandler.update_jumps.emit(max_jumps - jump_counter)	
